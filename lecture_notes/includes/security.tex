...
Blockwitholding is a deviation from the prescribed protocol. The goal is to build up a stock of blocks and release them publicly at well chosen times so as to fork the chain and make a part of the mining activities of competitors worthless, depending on which branch is followed up in the longer run. We focus here on a simplified version\footnote{For the sake of tractability.} of the original blockwitholding strategy called selfish mining and described in \citet{Eyal2014}.
\subsection{The selfish mining strategy}\label{ssec:selfish_mining}
Let Sam be a miner with hashpower $q\in(0,1)$ and define a discrete time stochastic process $(Z_n)_{n\geq0}$ that corresponds to the number of blocks in Sam's buffer. At each time step a block is found by the network and it belongs to Sam with probability $q$. The process $(Z_n)_{n\geq0}$ is a Markov chain with state space $E = \{0, 0^{\ast}, 1\}$. The state $0^{\ast}$ corresponds to the situation where a fork is ongoing, one branch is Sam's while the other branch belong to the honest miners. If Sam finds the next block then he appends it to his own branch. If the honest miners find the next block then they may append it to Sam's branch with probability $\gamma\in(0,1)$, a parameter refered to as Sam's connectivity. If $\gamma$ is high then Sam is well connected to the network which allows him to advertize his own branch to the other nodes that will then work on top of it. The honest miners append a block to the other branch with probability $1-\gamma$.  The dynamic of $(Z_n)_{n\geq0}$ is as follows. Consider some time $n\geq0$,
\begin{itemize}
...
Z_n = f\left[Z_{n-1},\xi_{n}\right] = \begin{cases}
...
\end{figure} 
Let $(\zeta_n)_{n\geq1}$ be a sequence of \iid Bernouilli variables with parameter $\gamma$ independent from the sequence $(\xi_n)_{n\geq1}$. We now define a reward collection process $(B_n)_{n\geq0}$ for Sam based on the values of $(Z_n)_{n\geq0}$, $(\xi_n)_{n\geq1}$, and $(\zeta_n)_{n\geq1}$. 
\begin{equation}\label{eq:gain_function_selfish}
B_n = f\left[Z_{n-1},\xi_n, \zeta_{n}\right] = \begin{cases}
0,&\text{ if } Z_{n-1} =0,\\
...
\end{cases}
\end{equation} 
We further define a reward collection process $(C_n)_{n\geq0}$ for the honest miners based on the values of $(Z_n)_{n\geq0}$, $(\xi_n)_{n\geq1}$, and $(\zeta_n)_{n\geq1}$. 
\begin{equation}\label{eq:gain_function_selfish}
C_n = f\left[Z_{n-1},\xi_n, \zeta_{n}\right] = \begin{cases}
0,&\text{ if } Z_{n-1} =1,\\
0,&\text{ if } Z_{n-1} =0^{\ast}\text{ }\&\text{ }\xi_{n} =1,\\
1,&\text{ if } Z_{n-1} =0\text{ }\&\text{ }\xi_{n} =0,\\
1,&\text{ if } Z_{n-1} =0^\ast\text{ }\&\text{ }\xi_{n} =0\text{ }\&\text{ }\zeta_{n} =1,\\
2,&\text{ if } Z_{n-1} =0^\ast\text{ }\&\text{ }\xi_{n} =0\text{ }\&\text{ }\zeta_{n} =0,\\
\end{cases}
\end{equation} 
Let $\mathcal{F}_n  =\sigma[(\xi_k, \zeta_k),\text{ }k\geq n]$ be a filtration. The processes $Z_n, B_n$ and $C_n$ are $\mathcal{F}_n$-adapted. We have that 
\begin{equation}\label{eq:mean_B_n}
\mathbb{E}(B_n) =\mathbb{E}[\mathbb{E}(B_{n}|\mathcal{F}_{n-1})] = \mathbb{P}(Z_{n-1} = 0^{\ast})(1-q)\gamma +2\mathbb{P}(Z_{n-1} = 0^{\ast})q +2\mathbb{P}(Z_{n-1} = 1)q  
\end{equation}
and
\begin{equation}\label{eq:mean_C_n}
\mathbb{E}(C_n) =\mathbb{E}[\mathbb{E}(C_{n}|\mathcal{F}_{n-1})] = \mathbb{P}(Z_{n-1} = 0)(1-q) +\mathbb{P}(Z_{n-1} = 0^{\ast})(1-q)\gamma +2\mathbb{P}(Z_{n-1} = 0^{\ast})(1-q)(1-\gamma)  
\end{equation}
Selfish mining may enable a miner to get a revenue higher than his fair share. The quantity to inspect is then the relative revenue of Sam defined as $\mathbb{E}(B_n) /[\mathbb{E}(B_n) +\mathbb{E}(C_n)]$ and the question is whether it holds that 
$$
\frac{\mathbb{E}(B_n) }{[\mathbb{E}(B_n) +\mathbb{E}(C_n)]}\geq q.
$$ 
The Markov chain $(Z_n)_{n\geq0}$ is irreducible on a finite state space, thuscd b it has a unique stationary distribution given by 
$$
...
$$
(q,\gamma)\mapsto\frac{\mathbb{E}(B_\infty) }{[\mathbb{E}(B_\infty) +\mathbb{E}(C_\infty)]}- q,
$$
the heatmap is provided in \cref{fig:relative_rev_map}.
\begin{figure}[!ht]
  \begin{center}
  \includegraphics[width=0.8\textwidth]{../Figures/relative_rev_map.pdf}
    \caption{Relative revenue as function of $q$ and $\gamma$.}
    \label{fig:relative_rev_map}
  \end{center}
\end{figure}
We observe that selfish mining is advantageous for some combinations of $q$ and $\gamma$ from a relative revenue point of view. Note that selfish mining entail a waste for all the nodes even the selfish one. The fact that the honest node tends to waste more than the selfish one may result in the honest nodes leaving the mining business or join the selfish miner and therefore making them prominent. Another consequence of selfish mining is the block production slowdown which will result in a downward adjustment of the cryptopuzzle difficulty. If miner resume following the protocol afterward, it might be beneficial. \\

\noindent The next section introduces a model for the financial reserves of miners to assess the risk and profitability of mining and selfish mining.
...
\end{enumerate}
\end{proof}
\noindent The main issue with formula \eqref{eq:value_function_finite_time_prop} is the lack of tractability. Formula \eqref{eq:value_function_finite_time_prop} is not a closed form expression because of the infinite serie. Note also that the evaluation of the higher order AG polynomials can result in numerical instabilities when using the recurrence relationship. The work around is to replace the fixed time horizon $t$ by an independent exponential random time horizon $T$ with mean $t$. We hence consider the ruin probability and expected profit at $T\sim \text{Exp}(t)$, defined by
...
\begin{equation*}
  \widehat{\psi}(u,t)=e^{-h(1/t + p\lambda)}\,\widehat{\psi}(u-ch,t) +\int\limits_0^h p\lambda\, e^{-s(1/t + p\lambda)}\,\widehat{\psi}(u-cs+b,t)ds,
  \end{equation*}
and 
\begin{eqnarray*}
  \widehat{V}(u,t)& =&e^{-h(1/t + p\lambda)}\,\widehat{V}(u-ch,t)+\int\limits_0^h\frac1t\, e^{-s(1/t + p\lambda)}\,(u-cs)ds\\
  &&\quad+\int\limits_0^h p\lambda\, e^{-s(1/t + p\lambda)}\,\widehat{V}(u-cs+b,t)ds.
  \end{eqnarray*}
Now we take the derivative with respect to $h$ and set $h=0$ to obtain
\begin{equation}\label{eq:ODE_psi}
c\widehat{\psi}'(u,t) + \left(\frac{1}{t} +  p\lambda\right)\widehat{\psi}(u,t) - p\lambda \widehat{\psi}(u+b,t)=0,
\end{equation}
 and
...
\end{equation}
Note that the derivative is with respect to the first argument and that
equations \eqref{eq:ODE_psi} and \eqref{eq:ODE_V} are advanced functional differential equations. Equation \eqref{eq:ODE_psi} is actually the homogeneous counterpart of \eqref{eq:ODE_V}. For \eqref{eq:ODE_psi} consider the form 
...
\end{cases}
\end{equation*}
where $A_1, B_1$ and $\rho$ are constant to be determined. We have that $B_1 = 0$ and $A_1= 1$. We then take the nonnegative solution $\rho^{\ast}$ of 
\begin{equation*}
ct\rho + \left(1+p\lambda t\right)-p\lambda te^{\rho b} = 0,
\end{equation*}
to ensure that $\underset{u\rightarrow \infty}{\lim}\psi(u,t) = 1$.
\begin{equation}\label{eq:potential_solution}
\widehat{V}(u,t) = Ae^{\rho u }+Bu + C,\text{ }u \ge 0, 
\end{equation}
where $A, B,C$ and $\rho$ are constants to be determined. Substituting \eqref{eq:potential_solution} in \eqref{eq:ODE_V} together with the boundary condition yields the system of equations 
\begin{equation*}
\begin{cases}
0&=ct\rho + \left(1+p\lambda t\right)-p\lambda te^{\rho b}, \\
...
\end{cases}
\end{equation*}
% We then have $A = -t(p\lambda b - c)$, $B = 1$, $C = t(p\lambda b - c)$ and $\rho$ is solution of Equation \eqref{eq:equation_rho}, which admits one negative and one positive solution. As $A<0$, we have to choose the negative solution $\rho^\ast<0$ in order to ensure $\vh(u,t)>0$. Substituting $A,B,C$ and $\rho^{\ast}$ in \eqref{eq:potential_solution}  yields the result.
\end{proof}
