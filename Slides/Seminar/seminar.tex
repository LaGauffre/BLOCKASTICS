...
\title[BLOCKASTICS]{Blockchain mining risk management}
...
\begin{itemize}
  \item Decentralized
  \item Public/private
  \item Permissionned/permissionned
  \item Immutable
...
\begin{column}{0.5\textwidth}
\scriptsize
LThe finanial reserves of a nonlife insurance company is given by
\begin{equation*}
...
\begin{itemize}
  ...
  \begin{itemize}
    \scriptsize
    \item[$\hookrightarrow$] Nonnegative random variables, \textbf{i.i.d.}, et indépendantes de $N_t$
  \end{itemize}
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
...
$$
The latter has two solutions on th ereal line, one negative and the other is negative. As $A<0$, we must take  $\rho^\ast<0$ to ensure that $\widehat{V}(u,t)>0$. Substituting $A,B,C$ and $\rho^{\ast}$ in \eqref{eq:potential_solution} yields the result.\\
...
\begin{itemize} 
  ...
  \item the relative difficulty $q\in(0,1)$ of finding a \textit{share} VS finding a prooper solution
...
\end{thebibliography}
  ...
  \begin{itemize}
    \scriptsize
    \item[$\hookrightarrow$] to wait for her proporrtion of submitted \textit{shares} to improve 
  \end{itemize} 
...
\scriptsize
Let us conndier randomized rewards
$$
...
\end{equation*}
where $R$ is the only solutoion to 
...
\scriptsize
BConditionning upon the events that occur during $(0,h)$. Four possibilities
\begin{itemize}
