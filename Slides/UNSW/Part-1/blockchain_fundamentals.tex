...
\begin{enumerate}
  ...
  \item Centralization 
  \begin{itemize}
    \item[TradFi] Bank are recorde keeper $\Rightarrow$ Cyber risk
...
\end{itemize}
Through the use of Smart Contract on the Ethereum blockchain.
\vspace{0.3cm}
\scriptsize
\begin{thebibliography}{1}
...
Exchange where you trade one token for another through a smart contract.
\end{tcolorbox}
 ...
 \begin{columns}
    ...
    \end{table}
  \end{columns}
  Lots of orders need to be issued
  \begin{itemize}
    \item Expensive
    \item Slow
  \end{itemize}
...
Instead of order book AMM relies on a simple mechanism based on 
\begin{itemize}
